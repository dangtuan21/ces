...
import AnnouncementForm from 'src/view/announcement/form/AnnouncementForm';
import AnnouncementService from 'src/modules/announcement/announcementService';

function AnnouncementFormModal(props) {
  ...
  const doSubmit = async (_, data) => {
    try {
      setSaveLoading(true);
      const { id } = await AnnouncementService.create(data);
        ...
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">
              {i18n('entities.announcement.new.title')}
            </h5>
            <button
              type="button"
              className="close"
