import React from 'react';
import { i18n } from 'src/i18n';
import actions from 'src/modules/feedback/importer/feedbackImporterActions';
import fields from 'src/modules/feedback/importer/feedbackImporterFields';
import selectors from 'src/modules/feedback/importer/feedbackImporterSelectors';
import ContentWrapper from 'src/view/layout/styles/ContentWrapper';
import Breadcrumb from 'src/view/shared/Breadcrumb';
import importerHoc from 'src/view/shared/importer/Importer';
import PageTitle from 'src/view/shared/styles/PageTitle';

function FeedbackImportPage() {
  const Importer = importerHoc(
    selectors,
    actions,
    fields,
    i18n('entities.feedback.importer.hint'),
  );

  return (
    <>
      <Breadcrumb
        items={[
          [i18n('dashboard.menu'), '/'],
          [i18n('entities.feedback.menu'), '/feedback'],
          [i18n('entities.feedback.importer.title')],
        ]}
      />

      <ContentWrapper>
        <PageTitle>
          {i18n('entities.feedback.importer.title')}
        </PageTitle>

        <Importer />
      </ContentWrapper>
    </>
  );
}

export default FeedbackImportPage;
